...
def create_huge_matrix():
    ...
    for i in range(0,n):
        for j in range(0,n):
            matrix[i,j] = np.sum((locations[i]-locations[j])**2)**.5

dist_matrix = [
    [   0.0, 2451,  713, 1018, 1631, 1374, 2408,  213, 2571,  875, 1420, 2145, 1972], # New York
...
solver=TSPSolver.TSPSolver()
solution = solver.main(dist_matrix)
print(solution)
path=solution["path"]
cost=solution['cost']

assert cost == 7293
solution = "->".join([cities[c] for c in path])
assert solution=="New York->Boston->Chicago->Minneapolis->Denver->Salt Lake City->Seattle->San Francisco->Los Angeles->Phoenix->Houston->Dallas->St. Louis"
