...
class Proj5GUI( QMainWindow ):
    ...
    def generateNetwork_nogui(self, diff, rand_seed, size):
        points = self.newPoints2(rand_seed, size) # uses current rand seed
    ...
    def newPoints2(self, seed, npoints):
        random.seed( seed )
        ptlist = []
        RANGE = self.data_range
        xr = self.data_range['x']
...
#		app.processEvents()
    ...
    def initUI( self ):
        ...
        self.size			= QLineEdit(SIZE)
        self.size.setFixedWidth(50)
        self.timeLimit		= QLineEdit(TIME)
        ...
        self.diffDropDown.setCurrentIndex(DIFFICULTY)
...
def manual():
    ...
    for size in range(10,41,2):
        rand_seed = random.randint(0,1000)
        scenario = w.generateNetwork_nogui(diff, rand_seed, size)
        w.solver.setupWithScenario(scenario)
        results = w.solver.branchAndBound(time_allowance=max_time)
...
if __name__ == '__main__':
    ...
    #s = Scenario(city_locations=5, difficulty=3, rand_seed=5)
